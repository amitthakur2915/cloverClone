...
const mongooseConnect = () => {
  ...
  const uri = "mongodb+srv://athakur852:amit852@cluster0.hthxuyj.mongodb.net/clovedb?retryWrites=true&w=majority";
  ...
  mongoose
    .connect(uri, {
      ...
      ssl: true,            // keep SSL ON for Atlas
      sslValidate: false,   // disable validation (helps on Windows)
      family: 4,            // force IPv4 to avoid DNS/IPv6 bugs
      serverSelectionTimeoutMS: 15000, // optional: 15 sec timeout
    })
    .then(async () => {
      ...
      if (!existingUser) {
        ...
        await new User({ username, email, password: hash, firstName, lastName, level: 'root' }).save();
      ...
      await Meeting.updateMany({}, { $set: { peers: [] } }).catch(console.log);
